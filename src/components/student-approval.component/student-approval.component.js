...
async function submitProposalDecisions(advisorId, acceptedList, rejectedList) {
  const result = await Helper.submitProposalDecisions(
    advisorId,
    acceptedList,
    rejectedList
  )
...
export default function StudentApproval() {
        ...
        {text === 'Proposals' && (
          <div className='decide-buttons'>
            <Button
              ...
              onClick={() => {
                acceptedList.push(student)
                var method = setStudentList
                removeStudentCard(student, studentList, method)
              }}
            >
              Accept
            </Button>
          ...
          <div className='decide-buttons'>
            <Button
              ...
              onClick={() => {
                ...
                var method = setAcceptedList
                removeStudentCard(student, acceptedList, method)
              }}
            >
              Undo
            </Button>
        ...
        {text === 'Rejected Proposals' && (
          <div className='decide-buttons'>
            <Button
              ...
              onClick={() => {
                studentList.push(student)
                var method = setRejectedList
                removeStudentCard(student, rejectedList, method)
              }}
            >
              Undo
            </Button>
  ...
  function getListView(text, list) {
    return (
      <div>
        ...
        >
          {list.map((tile) => (
            <ListItem key={tile.name} cols={tile.cols || 1}>
  ...
  function getPreviewView(list) {
    return (
      <div>
        ...
        >
          {list.map((tile) => (
            <ListItem key={tile.student.id} cols={tile.cols || 1}>
              <div className='approval-request'>
                <div>
                  <Avatar
                    alt='Remy Sharp'
                ...
                <div className='requester-info'>
                  <p className='requester-name'>
                    {tile.student.name + ' ' + tile.student.surname}
                  </p>
                  <p className='requester-department'>
                    {tile.student.department}
                  </p>
                </div>
                <h1>{tile.status}</h1>
              </div>
  ...
  return (
    <div className='student-approval'>
      ...
      <Button
        className='decide accept'
        disabled={!Boolean(acceptedList[0]) && !Boolean(rejectedList[0])}
        onClick={() => {
          submitProposalDecisions(userId, acceptedList, rejectedList)
        }}
      >
        Save Changes
      </Button>
